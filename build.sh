...
export ARGS="-j$(nproc) O=out"

# Init KSU and submodules
...
# Create out directory for kernel build
mkdir -p "${RDIR}/out"
...
if [ ! -d "${HOME}/toolchains/clang-r383902b" ]; then
    ...
    curl -s -LO "https://android.googlesource.com/platform/prebuilts/clang/host/linux-x86/+archive/0e9e7035bf8ad42437c6156e5950eab13655b26c/clang-r383902b.tar.gz"
    tar -xf clang-r383902b.tar.gz && rm clang-r383902b.tar.gz
    cd "${RDIR}"
fi

# ARM GNU toolchain setup
if [ ! -d "${HOME}/toolchains/gcc/arm-gnu-toolchain-14.2.rel1-x86_64-aarch64-none-linux-gnu" ]; then
    echo -e "\n[INFO] Cloning ARM GNU Toolchain\n"
    mkdir -p "${HOME}/toolchains/gcc" && cd "${HOME}/toolchains/gcc"
    curl -s -LO "https://developer.arm.com/-/media/Files/downloads/gnu/14.2.rel1/binrel/arm-gnu-toolchain-14.2.rel1-x86_64-aarch64-none-linux-gnu.tar.xz"
...
# Building function
build_ksu(){
    echo "[INFO] Configuring kernel..."
    make ${BUILD_OPTIONS} "${KERNEL_DEFCONFIG}" common.config ksu.config version.config
...
build_zip(){
    ...
    cp "${RDIR}/prebuilt-images/dt_exynos${SOC}.img" "${RDIR}/build/dt.img"
    cd "${RDIR}/build" || exit 1
    zip -cvf "MencretKernel-${MODEL}-${BUILD_KERNEL_VERSION}-stock-One-UI.zip" boot.img dt.img && \
    rm -f boot.img dt.img
    echo -e "\n[SUCCESS] Build Finished! Output: MencretKernel-${MODEL}-${BUILD_KERNEL_VERSION}-stock-One-UI.zip\n"
}
